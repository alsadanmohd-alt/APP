...
test('RLS, price integrity, overlap protection, private locations and chat',{skip:!(url&&key&&service)},async()=>{
 ...
 try{
  ...
  assert.ok((await owner.rpc('create_item',{p_title:'اختبار أمني',p_description:'غرض مؤقت للاختبارات الآلية',p_category:'تصوير',p_price:75,p_area:'الرياض',p_lat:24.781234,p_lng:46.634567,p_images:[],p_deposit:-1})).error);
  const id=ok(await owner.rpc('create_item',{p_title:'اختبار أمني',p_description:'غرض مؤقت للاختبارات الآلية',p_category:'تصوير',p_price:75,p_area:'الرياض',p_lat:24.781234,p_lng:46.634567,p_images:[],p_deposit:250}));itemIds.push(id);
  const publicItem=ok(await stranger.from('items').select('*').eq('id',id).single());assert.equal(publicItem.lat,24.78);assert.equal(publicItem.lng,46.63);
  assert.equal(ok(await stranger.from('item_locations').select('*').eq('item_id',id)).length,0);
  const start=new Date(Date.now()+5*86400000).toISOString().slice(0,10),end=new Date(Date.now()+7*86400000).toISOString().slice(0,10);
  const booking=ok(await renter.rpc('request_booking',{p_item:id,p_start:start,p_end:end}));
  assert.equal(ok(await renter.from('bookings').select('*').eq('id',booking).single()).total,150);
  assert.equal(ok(await renter.from('bookings').select('*').eq('id',booking).single()).deposit_amount,250);
  assert.ok((await renter.from('bookings').update({deposit_amount:0}).eq('id',booking)).error);
  ...
  assert.ok((await renter.from('bookings').update({total:1,status:'accepted'}).eq('id',booking)).error);
  ...
  const competing=ok(await stranger.rpc('request_booking',{p_item:id,p_start:start,p_end:end}));
  const decisions=await Promise.all([owner.rpc('change_booking',{p_booking:booking,p_status:'accepted'}),owner.rpc('change_booking',{p_booking:competing,p_status:'accepted'})]);
  assert.equal(decisions.filter(r=>!r.error).length,1);assert.equal(decisions.filter(r=>r.error).length,1);
  const winner=decisions[0].error?stranger:renter,loser=decisions[0].error?renter:stranger;
  assert.equal(ok(await winner.from('item_locations').select('*').eq('item_id',id).single()).lat,24.781234);
  assert.equal(ok(await loser.from('item_locations').select('*').eq('item_id',id)).length,0);
 }finally{
