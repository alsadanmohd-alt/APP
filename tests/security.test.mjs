...
test('RLS, contact validation, private locations, chat, edit/delete',{skip:!(url&&key&&service)},async()=>{
 ...
 try{
  ...
  // Display names are unique (case-insensitive, trimmed) across all users.
  assert.ok((await renter.from('profiles').insert({id:users[1],display_name:' أبو تجربة '})).error);
  ok(await renter.from('profiles').insert({id:users[1],display_name:'اسم آخر تمامًا'}));
